...
def remove_prefix(s3_uri: str) -> None:
    """Recursively delete everything under an ``s3://`` prefix (best-effort).

    Tries ``s5cmd rm prefix/*`` -> ``aws s3 rm --recursive`` -> boto3 batch
    delete. Used for checkpoint retention (``keep_last``); a failure to prune is
    logged, not raised, since stale checkpoints are harmless next to fresh ones.
    """
    base = s3_uri.rstrip("/")
    candidates: list[tuple[str, list[str]]] = []
    if have_s5cmd():
        candidates.append(("s5cmd", ["s5cmd", "rm", base + "/*"]))
    if have_aws():
        candidates.append(("aws", ["aws", "s3", "rm", "--recursive", base + "/"]))
    result, last_err, _not_found = run_cli_candidates(candidates)
    if result is not None or not candidates:
        if not candidates:
            _boto3_remove_prefix(base)
        return
    logger.warning(
        f"[arcstore] CLI delete {s3_uri} failed ({last_err}); falling back to boto3."
    )
    try:
        _boto3_remove_prefix(base)
    except Exception as e:  # noqa: BLE001
        logger.warning(f"[arcstore] failed to delete prefix {s3_uri}: {e}")


def _boto3_remove_prefix(s3_uri: str) -> None:
    import boto3
