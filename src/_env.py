...
def s3_retries() -> int:
    """Attempts for a transient S3 transfer before falling back / failing."""
    return max(1, env_int("ARCSTORE_S3_RETRIES", 3))
...
def cache_dir(subdir: str | None = None, *, create: bool = True) -> Path:
    """Resolve the local scratch root used for arcstore caches.

    Precedence: ``$ARCSTORE_CACHE_DIR`` > ``/local-ssd/arcstore`` (when present
    and writable) > ``<tempdir>/arcstore-cache``. Nothing touches the
    filesystem at import time; the directory is created only when
    ``create=True``.
    """
    env = env_opt("ARCSTORE_CACHE_DIR")
    if env:
        base = Path(env)
    elif _local_ssd_usable():
        base = Path(LOCAL_SSD_ROOT) / "arcstore"
    else:
        base = Path(tempfile.gettempdir()) / "arcstore-cache"
