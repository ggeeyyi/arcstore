...
* FUSE-mounted ``s3://`` prefix -> plain local glob over the mount with
  ...
  needed at runtime). This is the DEFAULT when a usable mount exists
  (``ARCSTORE_DATA_READ_POLICY`` defaults to ``auto``);
  ``use_mount=False`` (or ``read_policy="direct_s3"``) forces direct S3;
* direct ``s3://`` prefix (no mount, or ``direct_s3``) ->
  :class:`s3torchconnector.S3IterableDataset` (AWS CRT multi-threaded
  reads, rank/worker sharding via ``enable_sharding=True``);
...
from ..data.shuffle import reservoir_shuffle
from ..location import resolve

logger = logging.getLogger(__name__)

__all__ = ["ScatterPtDataset"]
