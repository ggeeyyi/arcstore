...
Default is ``auto`` (``ARCSTORE_WDS_READ_POLICY``): when the bucket is
FUSE-mounted, ``s3://`` shards open directly as mounted files
(``<mountdir>/key``); when there is no usable mount they become
``pipe:s5cmd cat ...``. Set ``read_policy="direct_s3"`` (or
``ARCSTORE_WDS_READ_POLICY=direct_s3``) to always stream from S3 even when a
mount exists.
"""
...
def _policy(value: str | None = None) -> str:
    return _read_policy(value, env_name="ARCSTORE_WDS_READ_POLICY", default="auto")
